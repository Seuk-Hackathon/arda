...
3차는 클래스 가중치를 **장수에 반비례**로 줬습니다. 역겨움(disgust)은 191장뿐이라 가중치가
**21.11**, 무표정(10,309장)은 **0.39** — **54배**입니다.

"역겨움(disgust)을 틀리면 54배 손해" 라고 가르치면 모델은 **애매할 때마다 역겨움이라
...
대가도 있습니다 — **무서움·역겨움(disgust) 재현율이 내려갔습니다**(76→60, 81→60). 드문
...
| 날짜 | 변경 | 이유 |
|---|---|---|
| 2026-09-17 | `disgust` 화면 표기 역겨움 → **긴장** (PR #286) | 면접 맥락에서 "역겨움"이 지원자에게 부적절하게 읽혔다. 모델 출력·학습 라벨은 바뀌지 않았다. |
