...
### 채점기·데이터 결함 3건 수정 (2026-09-14)

09-12 측정에서 드러난 채점기·데이터 결함을 고쳤다:

1. **`pending_pass` 가 gold 도구 집합으로 재유도** → `_meta.pending_action` 만 본다
   (데이터에 이미 라벨이 있다).
2. **`tool_match` 집합 완전 일치** → **부분집합 + 추가분이 전부 읽기 도구**면 통과.
   「먼저 찾고 실행」 (`search_applications` + `send_email`) 을 오답 처리하던 것을 고침.
3. **정답 데이터의 존재하지 않는 도구 `update_candidate_stage`** → `change_stage` 로
   교정 (dataset.test.jsonl case #19).

### Claude Haiku 4.5 재측정

| 판본 | 통과 | 정답 상한 | 상한 대비 |
|---|---|---|---|
| **수정 전** (09-12) | 16/23 = **69.6%** | 19/23 | 84.2% |
| **수정 후** (09-14) | 16/23 = **69.6%** | **20/23** | **80.0%** |

절대 통과 수는 같지만 정답 상한이 정확해졌다 (19→20). Claude 실패 7건은 대부분 **모델이
`change_stage` 를 부르지 않고 `search_applications` 만 부른 뒤 확인을 요청하는 패턴** 이다
— 운영 UX 상 옳은 행동인데(사용자 확정 없이 단계 바꾸지 않음), 정답은 한 턴에 write 를
바로 부르길 기대한다. 이 유형은 tool_match 완화로는 해결되지 않는다: 모델이 **더 많이**
부르는 게 아니라 **덜** 부르는 것이라서 부분집합 조건에서도 미달이다.

파인튜닝 Qwen3-8B (09-11 · 6/23 = 26.1%) 는 아직 재측정 안 됨 — GPU 재기동 필요.
채점기가 달라져 09-11 수치와 직접 비교 불가.
