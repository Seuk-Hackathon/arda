...
/// 알림 설정도 권한도 없고, 비밀번호는 **여기서 안 바꾼다**(메일 링크로
/// 웹에서 바꾼다 — 2026-09-16).
///
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        _Group(
          children: [
            /* 2026-09-16: 지원자에게도 비밀번호가 생겼다(백엔드 PR #268).
               **다만 이 앱에서 바꾸지는 않는다** — 설정·재설정이 메일 링크
               한 경로이고, 그 링크는 웹에서 열린다(앱에 딥링크가 없다).
               앱에 설정 화면을 또 만들면 두 벌을 들고 있게 된다.

               그래서 「준비 중」이 아니라 **어디서 하는지**를 적는다. 흐린
               줄에 이유가 없으면 고장인지 아직인지 구별이 안 된다. */
            const _Row(
              icon: Icons.lock_outline,
              title: '비밀번호 변경',
              sub: '로그인 화면에서 「비밀번호 설정 링크 받기」',
              trailing: '메일로',
            ),
          ],
        ),
