...
| n8n 이 웹훅은 받고(200) 그 뒤 죽음 | 행이 `queued` 로 남는다 — **자동으로 안 잡힌다** | 아래 쓸어 담기 |
| n8n 이 길게 죽어 있음 | — | `.env` 에 `MAIL_DISPATCH=smtp` → `docker compose up -d --force-recreate api` |

**밀린 메일 쓸어 담기** (`queued` 로 10분 넘게 남은 행을 SMTP 로 보낸다):

```bash
docker exec arda-api-1 python -m app.shared.mail_smtp
```

- `MAIL_PENDING_AFTER_MIN`(기본 10) 보다 오래 묵은 것만, 한 번에 `MAIL_FLUSH_LIMIT`(기본 50)건까지. 장애가 길었으면 여러 번 돌린다 — 한 번에 수백 통을 밀어 넣으면 지메일이 계정을 잠근다.
