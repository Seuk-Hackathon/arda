...
**WSL 에서 vite 를 띄우면 파일 변경이 화면에 반영되지 않는다** (2026-09-08). 프론트 개발 서버를 WSL 안에서 `/mnt/c/...` 를 보게 돌리면, **Windows 쪽에서 고친 파일이 inotify 로 안 올라와서 vite 가 변경을 못 본다.** 브라우저를 새로고침해도 **서버를 켤 때의 코드가 계속 나온다.**

이게 고약한 이유는 **틀린 화면이 그럴듯하게 그려지기 때문**이다. 고친 줄만 옛날 것이라, 코드를 읽으면 맞는데 화면은 다르게 나온다 — 원인을 코드에서 찾다가 오래 헤맨다. 실제로 그렇게 한 번 잃었다.

두 가지 중 하나로 푼다.

```bash
# ① 고칠 때마다 개발 서버를 다시 띄운다 (확실하다)
# ② 폴링으로 감시한다 — frontend/app/.env.local
VITE_DEV_POLL=1
```
